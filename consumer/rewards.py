...
def activeRewards(request):
	...
	data = db.users
	result = data.find_one({"user_id":userID},{"_id":False})
	if result is None:
		return basic_failure()
	data=db.rewards
	result1=data.find(projection={"_id":False})
	return basic_success({"total_earning":result['total_earning'] , "total_balance":result['total_balance'] , "total_redeemed":result['total_redeemed'],"redeemed":result['history']['redeemed_offers'] , "earning_list":result['history']['earnings'] , "active_rewards":result1 , "current_time":datetime.now()})
