...
def activeRewards(request):
	...
	data = db.users
	result = data.find_one({"user_id":userID},{"_id":False,"total_earning":True , "history":True})
	if result is None:
		return basic_failure()
	data=db.rewards
	result1=data.find()
	return basic_success({"earning":result['total_earning'] , "redeemed":result['history']['redeemed_offers'] , "earning_list":result['history']['earnings'] , "active_rewards":result1})
